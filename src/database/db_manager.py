...
class DatabaseManager(AbstractDatabaseManager):
    ...
    def save_backtest_results(self, results_dict):
        ...
        Args:
            results_dict: Dictionary from BacktestOrchestrator.get_results_dict()
        ...
        try:
            ...
            cursor.execute(
                ...
                INSERT INTO backtest_results (
                    symbol, strategy_name, timeframe, start_date, end_date, created_at,
                    total_profit_pct, sharpe_ratio, annual_return_pct, max_drawdown_pct,
                    profit_factor, total_orders, win_rate_pct, pl_ratio, winner_avg_pct,
                    loser_avg_pct, sortino_ratio, calmar_ratio, recovery_factor
                ) VALUES (?, ?, ?, ?, ?, ?,
                          ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    ...
                    metrics.get("winner_avg_pct"),
                    metrics.get("loser_avg_pct"),
            ...
            self.conn.commit()
            self.logger.info(
                "Saved backtest results (ID: %d) with %d trades",
                backtest_result_id,
                len(trades),
            )
